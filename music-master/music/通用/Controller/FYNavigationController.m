...
@interface FYNavigationController ()<PlayViewDelegate,UINavigationControllerDelegate,UIViewControllerTransitioningDelegate>
...
- (void)playButtonDidClick:(NSInteger)index {
    ...
    if ([[FYPlayManager sharedInstance] playerStatus]) {
        ...
        if ([[FYPlayManager sharedInstance] havePlay]) {
            [self showMassage:@"歌曲加载中"];
        }else
            [self showMassage:@"尚未加载歌曲"];
    }

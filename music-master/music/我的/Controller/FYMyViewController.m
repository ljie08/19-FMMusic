...
//    [UIApplication sharedApplication].statusBarStyle = UIStatusBarStyleDefault;
    ...
    [self.tracksVM getItemModelData:^(NSError *error) {
        NSLog(@"aaaaaaaaaaa");
        if (!self.tracksVM.rowNumber) {
            [self.mainTableView showNoView:@"暂时没有数据~" image:nil certer:CGPointZero];
        }
...
//    self.navigationController.navigationBar.barTintColor = [UIColor whiteColor];
    ...
    [self addNavigationWithTitle:string leftItem:nil];
    
    UIButton *rightBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    rightBtn.frame = CGRectMake(0, 0, 80, 30);
    [rightBtn setTitle:@"清空" forState:UIControlStateNormal];
    [rightBtn addTarget:self action:@selector(delAll) forControlEvents:UIControlEventTouchUpInside];
